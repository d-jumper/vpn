...
echo -e " ${LIGHT}- ${NC}Add User SSH & OpenVPN"
arfvpn_bar 'add_ssh'
echo -e ""
sleep 2
history -c
...
echo -e "           ⇱ \e[32;1m✶ Result SSH & OpenVPN Account ✶\e[0m ⇲ ${NC}" | tee -a /etc/arfvpn/log-create-user.log
echo -e "\033[0;34m└─────────────────────────────────────────────────────┘${NC}" | tee -a /etc/arfvpn/log-create-user.log
echo -e " " | tee -a /etc/arfvpn/log-create-user.log
echo -e "${NC}${CYAN}            ───✶ SSH & OpenVPN Account ✶─── $NC" | tee -a /etc/arfvpn/log-create-user.log
...
echo -e "  ${RED}•${NC} ${CYAN}SSH WS NONE SSL   : $ws2 $NC" | tee -a /etc/arfvpn/log-create-user.log
...
echo -e "" | tee -a /etc/arfvpn/log-create-user.log
echo -e "${NC}${CYAN}               ───✶ OpenVPN Config ✶─── $NC" | tee -a /etc/arfvpn/log-create-user.log
...
echo -e "" | tee -a /etc/arfvpn/log-create-user.log
echo -e "${NC}${CYAN}               ───✶ PAYLOAD CONFIG ✶─── $NC" | tee -a /etc/arfvpn/log-create-user.log
