...
echo -e " ${LIGHT}- ${NC}Add User Xray Trojan Websocket"
arfvpn_bar 'addtr'
...
clear
    ...
    echo -e "              ⇱ \e[32;1m✶ Result Xray Trojan Account ✶\e[0m ⇲ ${NC}" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "\033[0;34m└─────────────────────────────────────────────────────┘${NC}" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "${NC}${CYAN}            ───✶ Xray - Trojan Account ✶─── $NC" | tee -a /etc/arfvpn/log-create-user.log
    ...
    echo -e "  ${RED}•${NC} ${CYAN}Password          : ${uuid} $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "" | tee -a /etc/arfvpn/log-create-user.log
    ...
    echo -e "  ${RED}•${NC} ${CYAN}Path WS           : /trojan $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "  ${RED}•${NC} ${CYAN}Path GRPC         : /trojan-grpc $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "" | tee -a /etc/arfvpn/log-create-user.log
    ...
    echo -e "  ${NC}${CYAN}${trojanlink1} $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "  ${RED}•${NC} ${CYAN}────────────────────────────────── $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "  ${RED}•${NC} ${CYAN}Link NONE TLS     ➣ $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "  ${NC}${CYAN}${trojanlink2} $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "  ${RED}•${NC} ${CYAN}────────────────────────────────── $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "  ${RED}•${NC} ${CYAN}Link GRPC         ➣ $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "  ${NC}${CYAN}${trojanlink3} $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "  ${RED}•${NC} ${CYAN}────────────────────────────────── $NC" | tee -a /etc/arfvpn/log-create-user.log
    echo -e "" | tee -a /etc/arfvpn/log-create-user.log
